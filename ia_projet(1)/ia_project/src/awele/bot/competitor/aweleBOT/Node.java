...
public class Node {
    ...
    public Node getBestFollowingNode(){
        ...
        double bestNodeValue = UCT(childNodes.get(5).getSimulationWins(),childNodes.get(5).getSimulationCount(),getSimulationCount(),Math.sqrt(2));
        for(int i = 0; i < childNodes.size(); i++){
            double currentUCT = UCT(childNodes.get(i).getSimulationWins(),childNodes.get(i).getSimulationCount(),getSimulationCount(),Math.sqrt(2));
            if(currentUCT > bestNodeValue){
                bestNode = childNodes.get(i);
    ...
    public double getUCT(){
        return UCT(simulationWins,simulationCount, parentNode.simulationCount,Math.sqrt(2));
    }
    ...
    public double[] getWinningPercentages(){
        double[] percentages = {0,1,2,3,4,5};
        if(childNodes == null) return percentages;
        for(int i = 0; i < childNodes.size(); i++){
            if(childNodes.get(i).getSimulationCount() == 0) percentages[i] = 0;
            else percentages[i] = (double)childNodes.get(i).getSimulationWins()/(double)childNodes.get(i).getSimulationCount();
        }
        return percentages;
    }
